...
* Open data source
use "$data/workfile9015_USborn_all", clear
    ...
    * If we're doing everyone (suffix blank) add a nice extension for graph name
	...
	forvalues y = 1(1)3 {
	    ...
	    graph twoway (lpoly `occ`y'' pop2015  [aw=popwkage] if yr==1990, degree(1)  kernel(epan2) bwidth(5)) ///
            ...
            legend(style(zyx2)) legend(cols(5) rows(1)) legend(region(lstyle(none))) legend(region(lcolor(none))) legend(size(small)) ///
    ...
    grc1leg2  occ3 occ2 occ1, ///
		...
		ring(1) pos(6) legendfrom(occ1) saving("$gphdir/Fig_MSA_nobin_all_US.gph", replace)
		graph export "$workdir/fig/Fig_MSA_nobin_all_US.pdf", replace
