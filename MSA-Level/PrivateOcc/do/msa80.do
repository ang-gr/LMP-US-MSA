...
* only consider US-born
keep if bpl < 100

gen ctygp80 = 100*statefip+cntygp
joinby ctygp using "$data/80countygroup" 


/*2. Merge Autor's occ1990dd to the dataset then match to the PCS codes*/
* Match to occ1990dd first then match to PCS
merge m:1 occ using "$data/occ/occ1980_occ1990dd", keep(match) nogenerate
drop if occ1990dd == 417 | occ1990dd == 418 //police+firefighter
merge m:1 occ1990dd using "$data/occ/occ1990dd_pcs", keep(match) nogenerate

* Categories Dummies according to PCS def
gen occ_high=0
...
/*3 employment share*/
gen totwrkhr_indi = wkswork1*uhrswork*perwt
...
save "$data\shempl\shempl_1980", replace
